...
export default class WebGLRenderDriver implements RenderDriver {
  ...
  compileShader(shader: Shader, compiler: ShaderCompiler) {
    ...
    shader.attributes.forEach((value, key) =>
      info.attributes.set(
        key, gl.getAttribLocation(info.program!, value)));

    shader.uniforms.forEach(([value, type], key) =>
      info.uniforms.set(
        key, [gl.getUniformLocation(info.program!, value)!, type]));
  }
  ...
  bufferTarget(bufferType: BufferType) {
    ...
    return bufferType !== BufferType.TRIANGLE
      ? gl.ARRAY_BUFFER
  ...
  render() {
    ...
    this.engine.scene.getRootObjects().forEach(
      o => this.renderHierarchy(o));
  ...
  renderHierarchy(o: BaseObject) {
    ...
    o.children
      .filter(c => c.active)
      .forEach(c => this.renderHierarchy(c));
    ...
    info?.uniforms.forEach(([loc, type], key) => {
      switch (type) {
        case UniformType.MAT4:
          switch (key) {
            case 'projection': gl.uniformMatrix4fv(loc, false, this.projection); break;
            case 'modelView': gl.uniformMatrix4fv(loc, false, this.modelView); break;
            default:
          }
          break;
